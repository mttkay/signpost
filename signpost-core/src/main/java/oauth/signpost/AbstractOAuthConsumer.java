...
public abstract class AbstractOAuthConsumer implements OAuthConsumer {
	...
	public HttpRequest sign(HttpRequest request) throws OAuthMessageSignerException,
		...
		debugOut("signature: %s", signature);

		this.signingStrategy.writeSignature(signature, request, this.requestParameters);
		debugOut("Auth header: %s", request.getHeader("Authorization"));
		debugOut("Request URL: %s", request.getRequestUrl());

		return request;
	}

	protected void debugOut(String format, String... args) {
		Logger logger = LoggerFactory.getLogger(getClass());
		if (logger.isDebugEnabled()) {
			logger.debug(String.format(format, (Object[]) args));
		}
	}
